...
omarchy_host_test() {
  ...
  wait_for_guest_state "Theme Manager 0.5.10 is installed and loaded" 25 ssh_session \
    "omarchy-plugin-list --json | jq -e \
      'any(.[]; .id == \"io.github.mtolhuys.theme-manager\" and .enabled == true)' && \
     jq -e '.version == \"0.5.10\" and \
     ...
     [[ \$(omarchy-shell shell call io.github.mtolhuys.theme-manager runtimeIdentity '') == \"0.5.10\" ]]" || return 1
